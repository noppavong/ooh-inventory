 ...
 $("#register_date").inputmask("dd/mm/yyyy", {"placeholder": "dd/mm/yyyy"});
       ...
       var data = []; <?php
                    ...
                    <?php foreach($stock_transaction_detail->result() as $row) {
                        ...
                        if (sizeof($name_array) > 1) { ?>
                            ...
                            data.push({
                                ...
                                'previous_amount':<?php if($row->previous_amount){echo $row->previous_amount;}else{ echo 0;} ?>,
                                'amount': <?php if($row->cur_amount){echo $row->cur_amount;}else{ echo 0;} ?>,
            ...
            $('.save-btn').click(function (evt) {
                ...
                $.ajax({
                    url: "<?php echo base_url(); ?>stock/save",
                    data: {
                    ...
                    success: function (res) {
                        ...
                        if (res.result === 'ok') {
                           ...
                           window.location.replace('<?php echo base_url(); ?>stock/all');
                        } else {
                            if (res.transaction_id) {
                                $('input[name="stock_transaction_id"]').val(res.transaction_id);
                            }
