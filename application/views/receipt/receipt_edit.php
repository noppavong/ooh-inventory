<div class="row">
    ...
    <div class="col-md-12">
        ...
        <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <div class="box-body">
                <form role="form" method="post" action="<?php echo site_url('receipt/create_receipt'); ?>">
                    <!-- text input -->
                    ...
                    <?php } ?>
                     <div class="form-group">
                          <label>รายละเอียด(กรณีต้องการทำใบเสร็จแบบไม่ระบุสินค้า)</label>
                          <input type="text" id="sum_description" name="sum_description" class="form-control" />                                                             
                      </div>  
                     <div class="form-group">
                          <label>จำนวนเงิน(กรณีต้องการทำใบเสร็จแบบไม่ระบุสินค้า)</label>
                          <input type="text" id="sum_value" name="sum_value" class="form-control" />                                                             
                      </div> 
                    <div class="form-group">
                        <input type="number" name="other_percentage" class="form-control" placeholder="ปรับราคาสินค้า [1-100] %"/>
                    </div>   
                    <div class="form-group">
                      <label>gp</label>
                    ...
                    <div class="form-group">
                        ...
                        <div id="input_receipt" class="handsontable">
                        </div>
                    </div>
                    <button id="save-btn-receipt" class="btn btn-primary save-btn">Save</button>
    ...
    <script>
        $(function () {
            $('.cancel').click(function () {
                window.location.replace('<?php echo site_url("receipt/all"); ?>')
            });
            ...
            $barcode_string = implode(',', $barcode_array) ?>
                <?php
            if (!empty($receipt_id)) { ?>
                    $('input[name="receipt_id"]').val('<?php echo $receipt_id ?>'); <?php
            } ?>
                <?php
            ...
            <?php
            if (!empty($receipt_no)) { ?>
                    $('input[name="receipt_no"]').val('<?php echo $receipt_no ?>'); <?php
            } ?>
            <?php
            ...
            if (!empty($receipt_detail)) { ?>
                    <?php foreach($receipt_detail->result() as $row) {
                        $name_array = explode("\n", $row->product_name);
            ...
            initTable2('input_receipt',data,{discount:'<?php echo $percent_discount ?>'});
